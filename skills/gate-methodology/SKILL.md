...
Read-only. Never modify, create, or delete anything inside the research
folder. Reports go only to the output directory given to you.
Score by anchor, aggregate by formula. You only ever assign 0–3 anchor
...
Every judgment carries a location. Each anchor level you assign must
...
When torn between two anchor levels, assign the lower one and say in
...
Verdicts are computed, never judged. PASS iff score >= threshold and no
hard-fail rule triggered. You never decide a verdict; you calculate it.
No cached source content. When verifying sources, record URL, tier, and
...
All research content is confidential. It never leaves the reports.
Anchor scales
...
Aggregation rule for proxy/assumption rigor. An area typically contains
several distinct assumptions of uneven quality (e.g. one sensitivity-tested
figure among many bare "Assume:" values). Do not average or take a holistic
blend across them. Score the area's proxy/assumption rigor at the level of
its single weakest load-bearing assumption — the assumption whose failure
would most change the area's conclusion. A well-justified assumption elsewhere
in the same area does not average out an unjustified one that the conclusion
actually depends on.
Boundary rule between evidence chain and proxy/assumption rigor. These two
sub-scores must not double-penalize the same gap. Evidence chain judges
whether a stated conclusion traces back to a fact already established (cited
or computed) elsewhere in the document. Proxy/assumption rigor judges whether
a modeling input the analyst chose (an estimate, percentage, timeline,
multiple) is itself justified. An unsourced input used inside reasoning —
e.g. an assumed timeline that a conclusion is built on — is a proxy-rigor
issue, not an evidence-chain issue, even though the resulting conclusion
"depends" on it. Reserve evidence-chain penalties for conclusions that fail to
cite or compute from facts the corpus has already established.
